...
Чтобы скрипт работал 24/7, не зависел от того, закрыли ли вы консоль, и перезапускался в случае ошибки, рекомендуем использовать **Systemd** (для Linux) или **PM2**.

### Способ 1: Systemd (Рекомендуемый для Linux Ubuntu/Debian)
...
### Способ 2: Использование PM2

Если вам ближе экосистема Node.js, можете использовать `pm2`:

```bash
# Установка PM2 (если установлен NodeJS)
npm install pm2 -g

# Запуск python скрипта
pm2 start userbot_forwarder.py --name "max-forwarder" --interpreter .venv/bin/python
...
## Тестирование
В проекте есть вспомогательный скрипт `test_tg.py`. Он отдельно и напрямую использует `aiogram`, забирает настройки из `.env` файла и отправляет тестовое сообщение в ваш Телеграм чат. 
Используйте его, чтобы убедиться, что Telegram-бот добавлен в ваш чат и обладает правом писать сообщения: `python test_tg.py`.
